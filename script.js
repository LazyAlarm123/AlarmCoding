...
async function newElement() {
  list.innerHTML = '';
  let Data = await fetch(`http://localhost:${Port}/api/all`).then((response) =>
    response.json()
  );
  ...
  for (data of Data) {
    ...
    button.addEventListener('click', async function (event) {
      ...
      if (clickedElem.classList.contains('close')) {
        ...
        // console.log(index);
        await fetch(`http://localhost:${Port}/api/del/${index}`, {
          method: 'DELETE',
...
async function setAlarm() {
  if (isAlarmSet) {
    ...
    xhr.open('POST', `http://localhost:${Port}/api/saveAlarm`, true);
